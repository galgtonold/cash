...
def main() -> int:
    ...
    refit = {(f.family, f.backend_kind, f.operation): (f.a, f.b)
             for f in fit_all(args.csv_path, args.objective)}
    ...
    with open(args.csv_path, newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            if row["error"] or row["backend_kind"] != "disk":
                continue
            measured.append((row["family"], float(row["actual_size_bytes"]),
    ...
    print(f"{total} decisions scored against measured restore times "
          f"({len(measured)} cells x {len(EXEC_TIMES)} body times)\n")
    for label in ("shipped", "refit"):
        wrong = sum(tally[label].values())
        print(f"  {label:>8}: {wrong:>3} wrong ({wrong/total:5.1%})   "
