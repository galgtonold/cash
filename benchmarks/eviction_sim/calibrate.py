...
def describe(rows, label):
    ...
    if len(rows) > 2:
        print(
            f"  log-sd exec {statistics.pstdev(le):.2f}   log-sd size {statistics.pstdev(ls):.2f}   "
...
def main():
    ...
    ap.add_argument("--pattern", default="*-cold-*.json", help="cold runs only: their execution_time is real compute")
