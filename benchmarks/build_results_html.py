...
- src/cash/cost_model.py                (fitted constants)
...
NOTEBOOKS = [
    ...
    # and reported, no pass criterion yet: on 2026-09-14 cold ran at 7x and
    # 13x of uncached (down from 19x and 35x).
    ("synthetic_many_inputs", "benchmarks/synthetic_many_inputs.ipynb"),
    ("synthetic_folder_loop", "benchmarks/synthetic_folder_loop.ipynb"),
]
...
def load_coeffs() -> dict[tuple[str, str, str], tuple[float, float]]:
    from cash.cost_model import _COEFFS
...
def build(matrix: list[dict], coeffs: dict, per_nb: dict, criteria: list[dict]) -> str:
    ...
    parts.append(
        ...
        "Cost-model constants live in <code>src/cash/cost_model.py</code>; "
