...
| `file_hash_full_max_bytes` | `CASH_FILE_HASH_FULL_MAX_BYTES` | `8388608` (8 MiB) | Largest tracked file hashed IN FULL when checking freshness. Above it, three head/middle/tail regions plus the timestamps decide — cheap on a multi-GB parquet, and blind to a same-size interior edit whose mtime is then restored (`cp -p`, `rsync -a`). Linux and macOS catch that through the inode change time; Windows does not. Checking is memoized per process, so the second and later looks at an unchanged file cost a `stat`; [`CACHE-FRESHNESS-COST`](../warnings.md#cache-freshness-cost) reports when what remains is a bad trade. Raise this to hash more in full, at about 0.72 ms per MiB on every cache hit that depends on the file. |
