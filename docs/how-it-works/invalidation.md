...
<!-- claim: cash/notebook/server_discovery.py:_NOTEBOOK_PATH_CACHE_TTL == 300.0, cash/notebook/server_discovery.py:_read_notebook_code_cells @f6b1396e, cash/notebook/upstream/checker.py:UpstreamChecker._load_notebook_and_find_cell @9f1fee66, cash/notebook/upstream/checker.py:UpstreamChecker._resolve_notebook_path @0628dcf7 -->
...
<!-- claim: cash/notebook/upstream/checker.py:UpstreamChecker._notebook_cells_for @97d09e98, cash/notebook/server_discovery.py:warn_notebook_not_found_once @32e76860 -->
A failed lookup is also what triggers Cash's once-per-session notice that "upstream dependency tracking is disabled for this session" — and that notice fires the moment discovery fails, before Cash has attempted a cell read from any source, live or file. When a live reader *does* answer, though, the upstream simulation proceeds on the cells it supplied even though no path was ever resolved — so the notice can go out on a check that is not, in fact, fully disabled. What actually stays disabled in that case is narrower than the notice says: the notebook-wide search for a helper function's or class's source defined in another cell has no live-reader fallback of its own — it reads only from the resolved path, and comes back empty, silently, whenever that path failed to resolve.
...
<!-- claim: cash/notebook/upstream/checker.py:UpstreamChecker @ba7e70bc, cash/notebook/upstream/simulator.py:NotebookSimulator @8ffd10e8, cash/notebook/server_discovery.py:_read_notebook_code_cells @f6b1396e broad="the simulation story is the two orchestrating classes, not one method" -->
