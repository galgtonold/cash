...
When `use_locking=True`, the miss path routes through `_compute_with_lock` (`src/cash/core.py:2557`) instead of calling the compute closure directly (`src/cash/core.py:1503-1504`). The helper does three things:

1. **Acquire `self.backend.lock(cache_key)`** as a context manager (`src/cash/core.py:2574-2576`).
2. **Re-read the cache inside the lock** (`src/cash/core.py:2581`). If the entry now exists (because a concurrent caller wrote it while the current thread was waiting on the lock), return the cached value — `_validate_ttl` runs, then `_wrap_iterator_hit` reconstructs iterator chunks if needed.
3. **Otherwise compute and store inside the lock** (`src/cash/core.py:2606`).
...
Acquisition, compute, and release are deliberately separated. If **acquisition** raises anything at all, the helper emits a `CashCacheIneffectiveWarning` via `_warn_lock_failed` and falls through to an unlocked compute (`src/cash/core.py:2577-2579`) — the user's function still runs and the result is still cached, so the redundancy guarantee is best-effort, not absolute. If **release** fails, it is logged at debug and swallowed (`src/cash/core.py:2608-2611`) so a release error can't re-run the compute. A **compute** exception propagates normally and is never mistaken for a lock failure.
...
The flag is a `Cash`-instance option (`src/cash/core.py:463,526`), not a per-decorator one. All functions registered through this instance go through the lock path on misses; switch instances if you want a mix.
...
All of them. The base-class implementation at `src/cash/backends/_base.py:369` is a **real per-key `threading.RLock`**, drawn from a process-local registry (`_inprocess_key_lock`, `src/cash/backends/_base.py:392-419`) that is created lazily and guarded by a per-instance meta-lock, with a module-global bootstrap lock (`_base.py:23`) serializing the first-use race. Every backend that doesn't override `lock()` inherits this — so `use_locking=True` honors its promise on the default `Cash()`.
...
|---|---|---|
| `CacheBackend` (base) | **Defines** (`src/cash/backends/_base.py:369-390`) | Per-key `threading.RLock` from a process-local registry. **In-process** single-flight. Inherited by everything below except Redis. |
| `RedisBackend` | **Overrides** (`src/cash/backends/redis_backend.py:199-209`) | Distributed lock via `client.lock(lock_name, timeout=60, blocking_timeout=10)` — Redis's standard `SET NX PX` recipe. Lock name is `{prefix}lock:{cache_key}`. **Cross-process, cross-host.** |
| `SQLiteBackend` | Inherits — **deliberately** | Gets the base in-process `RLock`. A `NOTE` at `src/cash/backends/sqlite_backend.py:311-314` explains why there is no override: the backend's internal `self._lock` guards *storage integrity* (concurrent DB writes), which is a distinct concern from *compute single-flight*. |
...
`use_locking=True` **is supported on the async path**, via in-process single-flight rather than `_compute_with_lock` (`src/cash/core.py:1560-1691`). Concurrent awaits of the same cache key coalesce: the first awaiter (the *leader*) registers an `asyncio.Event` in `self._async_inflight` (`src/cash/core.py:524`), computes, and stores; other awaiters of the same key (the *followers*) `await` the event and then read the stored result. If the leader stored nothing — `cache_if` rejected the value, or the compute raised — followers fall through and compute themselves, so correctness is never traded for the optimization.
...
- `f.cache_info()` reports hit/miss counts. Under contention without locking, you'll see misses that "should" have been hits — count them across runs to estimate redundancy. Lock failures land in `cache_info()['warnings']` per the `_warn_once` rolling log (`src/cash/core.py:1787`).
...
| Surface | Effect |
|---|---|
| `Cash(use_locking=True)` | Route the miss path through `_compute_with_lock` for every function registered against this instance (`src/cash/core.py:463,526,1503-1504`). On the async path, enables in-process single-flight (`src/cash/core.py:1560-1691`). |
| `Cash(use_locking=False)` (default) | Skip the lock helper; compute closure runs directly. |
| `CacheBackend.lock(key)` (base) | Per-key `threading.RLock` from a process-local registry — **in-process single-flight** (`src/cash/backends/_base.py:369-419`). Inherited by every backend except Redis. |
| `RedisBackend.lock(key)` | Returns `client.lock(f"{prefix}lock:{key}", timeout=60, blocking_timeout=10)` — a real **distributed** lock (`src/cash/backends/redis_backend.py:199-209`). The only backend that coordinates across processes. |
...
| `CashCacheIneffectiveWarning` (lock failed) | Emitted once per `(func_name, "lock_failed")` when lock acquisition raises (`_warn_lock_failed`, `src/cash/core.py:2805-2824`). The call proceeds without the lock. |
