...
Detection happens at `src/cash/core.py` — `_is_one_shot_iterator` returns True when `iter(value) is value`. Generators, generator expressions, `map`/`filter`/`zip` results, and any custom class whose `__iter__` returns `self` all qualify. Plain collections (`list`, `dict`, `set`, `tuple`, `range`, `str`) fail that test and are cached as ordinary blobs — they are already reusable.
...
The write path lives in `_write_chunks` at `src/cash/core.py`. The loop is:
...
3. When `len(buffer) >= chunk_max_items` **or** `buffer_bytes >= chunk_max_bytes`, flush the buffer to the backend under key `f"{cache_key}:chunk_{i}"`, increment `i`, and reset the buffer (`src/cash/core.py`).
4. When the source iterator stops, flush any tail buffer the same way.

On exhaust, Cash writes a **manifest entry** at the canonical `cache_key` carrying `iterator_storage="chunked"`, `n_chunks`, and `total_items` (`src/cash/core.py`). The manifest is what the hit path reads first.

Defaults (`src/cash/core.py`):
...
- **Async generators** (`async def gen(): yield ...`). The decorator detects them at `src/cash/core.py` and returns the function unwrapped with a `CashCacheIneffectiveWarning`. See [Async Caching](async-caching.md) for the patterns that work with async code. An ordinary `async def` function that **returns** (rather than `yield`s) a sync iterator does chunk-store normally — the async wrapper at `src/cash/core.py` dispatches through the same `_write_chunks` path.
...
On a cache hit, the dispatch at `_wrap_iterator_hit` (`src/cash/core.py`) reads `metadata['iterator_storage']` and returns a **fresh** iterator instance:

- Single-chunk hits return `_ListCachedIterator(items)` — a thin wrapper over `iter(list)` that streams the cached list (`src/cash/core.py`).
- Multi-chunk hits return `_ChunkedCachedIterator(cash, cache_key, n_chunks)` — a lazy iterator that fetches one chunk at a time (`src/cash/core.py`).
...
- **Chunks are loaded lazily.** `_ChunkedCachedIterator.__next__` only calls `self._cash.backend.get(chunk_key)` when iteration crosses into a new chunk (`src/cash/core.py`). If the caller breaks out early, unreached chunks are never read from disk.
...
TTL is honored uniformly: each chunk inherits the manifest's TTL (`src/cash/core.py, 2135, 2167`), so `Cash.cleanup()` reclaims expired chunks alongside the expired manifest. Test reference: `test_chunked_chunks_inherit_manifest_ttl` at `tests/test_core/test_iterator_caching.py:973`.
...
The hit path's metadata flag `iterator_storage='chunked'` is what distinguishes a chunked manifest from a scalar list return that happens to live under a similar key (`src/cash/core.py`). Anything without that flag is returned as-is, which is the right thing for non-iterator return types.
...
- **Materialization on the first call.** The cache-miss path drains the user's iterator to completion before returning the replay wrapper (`src/cash/core.py`). If the caller plans to consume only the first ten items, the first call still does the full work. Subsequent calls benefit from the cache; the first one does not. Partial consumption is fully supported on cache hits — only chunks the caller reaches are loaded. Test reference: `test_chunked_iterator_partial_consumption_then_hit` at `tests/test_core/test_iterator_caching.py:806`.
...
| `_ListCachedIterator` | Internal | Single-chunk replay wrapper. Streams a materialized list; iter-self; supports `close()`. (`src/cash/core.py`) |
| `_ChunkedCachedIterator` | Internal | Multi-chunk lazy replay. Fetches one chunk at a time; iter-self; terminates cleanly on missing chunks; supports `close()`. (`src/cash/core.py`) |
| `metadata['iterator_storage'] = 'chunked'` | Backend metadata | Flag the hit path reads to choose `_ChunkedCachedIterator` over a single-blob return (`src/cash/core.py`). |
