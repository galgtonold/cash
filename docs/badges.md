...
6. **Overhead** — the bottom row labeled `overhead`, split into up to five sub-parts: `upstream check` (re-checking upstream cells), `cache write` (Cash hashing and serialising this cell's results into the cache — the dominant overhead for large objects), `badge setup` and `badge render` (building and updating the badge display), and `other` (everything else). Each per-row time chip above shows a statement's pure compute (or restore) time, so the `cache write` cost lives here rather than being folded into those chips. Shows when the breakdown is meaningful relative to cell time.
