...
### Decision
All persistent lineage state lives behind one seam: `LineageStore` (in `cash.notebook.lineage_store`), held as `TrackingState.lineage`. Writes go through `record(var, hash, value=...)` (fresh computation / restore) or `reset_to(var, hash)` (downstream-advancement resync). Reads use `resolve_lineage(var, lineage, value=, virtual=, compute_hash_fn=)` (or `LineageStore.resolve` against the store), the canonical implementation of the priority ladder (virtual → store → `_cash_lineage_hash` → compute_hash → str fallback); the cache key resolves its inputs through it.
...
### Consequences
- Lineage writes call `state.lineage.record(...)`, `reset_to(...)`, `discard(...)` or `clear()`. `state.variable_lineage` is a read-only view of the store, so a direct dict write (`state.variable_lineage[var] = h`) raises `TypeError` instead of silently skipping the attribute.
...
### Amendment (2026-09-24): superseded in part by later splits

The package decision stands; the layout above no longer matches the code.

- `StatementProcessor` no longer owns four siblings. It composes one
  collaborator per concern, each in its own module under `statement/`: the
  store (`store.py`), the hit server (`hit.py`), provenance records
  (`records.py`), call routing, randomness, mutation classification, the
  rebuild-cost ledger and the perpetual-miss guard, besides the four above.
- The package re-exports `StatementProcessor`, `ProcessResult`,
  `DecoratorCallMetric`, `StatementCacheMetadata` and `is_control_body`, and
  no private test-patching symbols: `TeeWriter` lives in `notebook/_tee.py`
  and stream capture in `statement/capture.py`.
- The file-dependency snapshot helpers live in
  `src/cash/tracking/file_dep_snapshot.py`, not `notebook/file_dep_snapshot.py`.
- The TypedDicts live in `statement/results.py`; `StatementCacheMetadata`
  is a frozen dataclass in `statement/_metadata.py` (ADR-014).
...
### Amendment (2026-09-24): superseded in part by later changes
