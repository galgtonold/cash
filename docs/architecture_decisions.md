...
**Remaining: the literal virtual variable — committed as the target architecture (not just perf).** The three mechanisms above (post-state table, chain-rebuild, observer) plus the CAS-223 epoch are four special rules whose *seams* produced the bugs found along the way (the combined case, the args-derived edge, the stale-seed guard). Modeling the RNG as a real variable in the lineage graph makes position-awareness, invalidation, and chain reconstruction fall out of the trusted variable machinery instead of being re-derived per rule — fewer seams, less to maintain. That maintainability win is the reason to do it, over and above turning CAS-226's spurious miss into a hit.
...
### Checkpoint A landed (`77ee74c`) — key-side representation migrated

The runtime cache key now folds the draw-vs-seed dependency (CAS-223) through the virtual variable `__cash_rng__<module>` in `variable_lineage`, via the ordinary input-lineage channel, **replacing** the `rng_epoch_fingerprint` side-channel in the key. The variable is added only to the *local* key-input set in `_analyze_and_hash`, never to the returned `inputs`/`outputs` — so cacheability, mutation and capture never process a phantom variable. This is a cleaner boundary than the plan's step-1 "inject into inputs/outputs then filter it back out in step 5": there is nothing to filter because nothing is polluted, and the earlier throwaway injection's 6-test blast radius does not recur (those 6 pass). This is deliberately the *transitional* fifth-rule state the note above warns about: `_rng_seed_epochs` is kept in lockstep because restore.py's replay guard still reads it. Behaviour-preserving; notebook 2075 + core 434 + 37 RNG integration + 131 RNG unit green.

### Plan refinement discovered while landing A — the simulator is in scope for step 3

Step 3 ("position-aware key → CAS-226 hits") is **entangled with the simulator**, which the original plan understated. The simulator (`upstream/virtual_lineage.py`) has **four** `compute_cache_key` sites and **none** pass the RNG component — draw keys are *runtime-only* today, a pre-existing asymmetry the checker compensates for with `_prepend_stale_seed_cells` and `_restore_position_rng_state` (re-execution) rather than by matching keys. So making the runtime key position-aware does **not** by itself make CAS-226 *hit*: the simulator must symmetrically reconstruct `__cash_rng__<module>` as it walks upstream cells (seed → advance the var; draw → read it) and include it in the same draw keys, or runtime and simulator keys diverge (violating the cache-key unification invariant). That simulator reconstruction is the load-bearing remaining work — and it is exactly what lets the two checker mechanisms (chain-rebuild, position-restore) and restore.py's epoch guard retire together, since the simulator would then reproduce a draw's position-correct key directly. Steps 4 and 6 remain as written.
