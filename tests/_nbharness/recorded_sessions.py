"""Real user sessions, shrunk to a small dataset, for ``test_recorded_sessions``.
...
DEMAND_FORECAST = Session(
    ...
    steps=(
        ...
        Edit(
            "features",
            lambda s: s.replace("panel['holiday'] = panel['date'].isin(nat).astype(int)\n", _REGIONAL_HOLIDAYS),
        ),
        ...
        Edit("features", lambda s: s.replace("s.rolling(7, min_periods=1)", "s.shift(1).rolling(7, min_periods=1)")),
        Run("chart"),
...
def _orders() -> str:
    ...
    for k in range(1, 601):
        ...
        rows.append(
            f"{k},{u},{_AB_START + np.timedelta64(day, 'D')}T12:00:00,{revenue},{refunded},"
...
READOUT_FILES = tuple(
    ...
    + [(f"data/events/events_{_AB_START + np.timedelta64(d, 'D')}.csv", _events_day(d)) for d in range(_AB_DAYS)]
)
...
READOUT = (
    ...
    ("overall", "overall = bootstrap_diff(users, METRICS)\nprint(overall.round(4).to_string())\n"),
    (
        "cuped",
...
AB_READOUT = Session(
    ...
    steps=(
        ...
        Edit(
            "segments",
            lambda s: s.replace(
                "SEGMENTS = ['platform', 'country']", "SEGMENTS = ['platform', 'country', 'user_type']"
            ),
        ),
        Run("charts"),
...
def _tickets_month(month: int, relabel: bool = False) -> str:
    ...
    for k in range(60):
        ...
        rows.append(
            {
                "id": month * 1000 + k,
                "created": f"2025-{month:02d}-{1 + k % 28:02d} 10:{k % 60:02d}",
                "channel": ("email", "chat", "web")[k % 3],
                "subject": words[0],
                "body": "\n".join(body),
                "queue": label,
            }
...
_MERGE = (
    "merge_map = queues.dropna(subset=['merged_into']).set_index('queue')['merged_into'].to_dict()\n"
