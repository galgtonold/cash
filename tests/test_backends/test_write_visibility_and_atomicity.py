...
class TestAtomicWrites:
    def test_target_is_untouched_when_the_rename_fails(self, tmp_path, monkeypatch):
        """Content goes to a temp file; the target appears only at the rename.

        Proves the payload is never streamed into the live path, which is what
        let a concurrent reader see a half-written file.
        """
        ...
        assert not os.path.exists(target), "payload was written into the live path"
    ...
    def test_existing_entry_survives_a_failed_rewrite(self, tmp_path, monkeypatch):
        """A failed overwrite must not destroy the entry already there."""
        backend = FileBackend(cache_dir=str(tmp_path / "c"))
        backend._ensure_initialized()
        target = os.path.join(backend.cache_dir, "probe.data")
...
class TestTruncatedEntryDegrades:
    def test_truncated_data_file_is_a_miss_not_a_crash(self, tmp_path):
        """``EOFError`` must degrade to a miss.

        cash no longer writes partial files, but a cache directory can still
        hold one from an older version, a killed process, or a full disk.
        ``EOFError`` subclasses Exception directly - not OSError, not
        ValueError - so it escaped ``get()``'s handler and surfaced to the user
        as "Ran out of input" instead of a recompute.
        """
        backend = FileBackend(cache_dir=str(tmp_path / "c"))
        backend.set("k", {"big": list(range(1000))})
        assert backend.get("k")[1] is not None  # settles the write

        meta_path, data_path = backend._get_paths("k")
        # ZERO length, not half: that is the file a reader saw the instant a
        # non-atomic write created it, and it is what raises the exact CI
        # signature "EOFError: Ran out of input". A half-written pickle raises
        # UnpicklingError instead, which this handler already caught - testing
        # that case would prove nothing about EOFError.
    ...
    def test_truncated_metadata_file_is_a_miss_not_a_crash(self, tmp_path):
        backend = FileBackend(cache_dir=str(tmp_path / "c"))
        backend.set("k", "v")
        assert backend.get("k")[1] == "v"

        meta_path, _ = backend._get_paths("k")
        with open(meta_path, "wb"):  # zero-length -> pickle.load raises EOFError
            pass
        backend._metadata_cache.pop("k", None)
