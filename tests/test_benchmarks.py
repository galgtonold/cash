...
class TestLineageHashPerformance:
    ...
    def test_lineage_hash_with_inputs(self):
        ...
        def fn():
            sorted_inputs = ":".join(f"{k}={v}" for k, v in sorted(input_lineages.items()))
...
class TestSerializationPerformance:
    ...
    @pytest.mark.skipif(not any(m in sys.modules for m in ("pandas", "numpy")), reason="pandas/numpy not imported")
