...
def _run_cell(magics, code):
    ...
    with patch('cash.notebook.upstream.checker.get_notebook_cells') as cells, \
...
class TestTheHeaderIsRead:
    ...
    def test_the_file_a_loop_header_reads_reaches_what_the_loop_built(
    ...
    def test_a_header_that_reads_nothing_records_nothing(
            self, magics_fixture, tmp_path):
...
class TestAMovedFileIsCheckedEitherWay:
    ...
    def _simulate(tmp_path, entry_lineages, files, stored_component):
        ...
        state.control_outcomes[hashlib.sha256(code.encode('utf-8')).hexdigest()] = (
            entry_lineages, {'OUT': 'produced-lineage'}, frozenset(files),
            stored_component,
        )
        vl = VirtualLineage(shell, Cash(backend=InMemoryBackend(),
                                        register_magic=False), state)
        stale: set[str] = set()
        vl._simulate_one_control_unit(
            ast.parse(code).body[0], {'DATA': 'a-simulated-lineage'}, set(),
            [], {}, set(), set(), vars_with_stale_files=stale,
        )
        return stale

    def test_a_moved_file_is_stale_although_the_input_lineages_disagree(
            self, tmp_path):
    ...
    def test_a_matching_outcome_with_its_files_in_place_is_still_trusted(
            self, tmp_path):
        ...
        stale: set[str] = set()
        vl._simulate_one_control_unit(
            ast.parse(code).body[0], virtual, set(), [], {}, set(), set(),
