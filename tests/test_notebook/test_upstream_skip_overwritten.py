...
class TestSkipOverwrittenVarUnit:
    ...
    def test_fully_redefined_var_stops_cascade(self, mock_cells_ids, mock_cells, upstream_checker):
        ...
        assert len(result) == 1, f"Expected 1 statement to execute, got {len(result)}: {result}"
        assert "x = 'second'" in result[0], f"Expected 'x = second' to be scheduled, got: {result}"
        # The first definition (with print) should NOT be scheduled
        assert not any("first" in s for s in result), f"Earlier definition should NOT be scheduled: {result}"
    ...
    def test_mutation_still_needs_earlier_def(self, mock_cells_ids, mock_cells, upstream_checker):
        ...
        assert len(result) == 2, f"Expected 2 statements (definition + mutation), got {len(result)}: {result}"
    ...
    def test_try_except_skipped_when_later_full_redef(self, mock_cells_ids, mock_cells, upstream_checker):
        ...
        for stmt in result:
            assert "ValueError" not in stmt, f"try/except block should NOT be scheduled: {stmt}"
            assert "asdf" not in stmt, f"try/except block should NOT be scheduled: {stmt}"
