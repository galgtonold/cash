...
def test_a_repair_nameerror_names_the_line_cash_did_not_re_run():
    ...
    m = _fail_with(_Checker({}), "sales.loc[in_cents, ['price']] /= 100",
                   NameError("name 'in_cents' is not defined"), ["import cash", cell])
...
def test_a_repair_keyerror_names_the_loop_that_fills_the_dict():
    cells = ["results = {}\nfor name in ['logreg']:\n    results[name] = name",
             "fig = draw(results['logreg'])"]
    m = _fail_with(_Checker({}), "fig = draw(results['logreg'])", KeyError('logreg'), cells,
                   scheduled=["results = {}"])
...
def test_a_repair_attributeerror_names_the_column_it_reads():
    ...
    m = _fail_with(_Checker({}), "weekly = sales[sales.week <= 3]",
                   AttributeError("'DataFrame' object has no attribute 'week'"), cells)
    assert "'week' is set by \"sales['week'] = sales['date']\" (cell 1)" in m, m


def test_a_statement_that_ran_before_is_not_blamed_on_the_user():
    m = _fail_with(_Checker({'best': "best = pick(results)"}), "best = pick(results)",
                   KeyError('x'), ["best = pick(results)"])
