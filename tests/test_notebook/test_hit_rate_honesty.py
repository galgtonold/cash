...
def _trivia(n, code_prefix="print"):
    ...
    return [{"status": CacheStatus.COMPUTED, "execution_time": 0.0002, "code": f"{code_prefix}({i})"} for i in range(n)]


def _expensive_hits(n, saved=13.5):
    return [{"status": CacheStatus.RESTORED, "saved_time": saved, "code": f"model_{i} = fit()"} for i in range(n)]


def _expensive_misses(n, cost=13.5):
    return [{"status": CacheStatus.COMPUTED, "execution_time": cost, "code": f"model_{i} = fit()"} for i in range(n)]
