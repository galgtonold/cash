...
def progress_probe(monkeypatch):
    """A BadgePresenter whose badge publishes are recorded instead of displayed."""
    from cash.notebook.ipython.badges import BadgePresenter

    badges = BadgePresenter(shell=None, cash=None)
    badges.MIN_RENDER_INTERVAL = 0.05  # keep the tests quick

    published: list[dict] = []
    lock_violations: list[str] = []

    # `arm_progress`'s `fire()` calls `_build_html` (outside the
    # lock) and then `_publish_html` (inside it) separately -- not
    # `render` as a single call -- so both halves need
    ...
    def fake_build(
        ...
        if badges._progress_lock.acquire(blocking=False):
            badges._progress_lock.release()
        else:
    ...
    badges._build_html = fake_build  # type: ignore[assignment]
    badges._publish_html = fake_publish  # type: ignore[assignment]
    return badges, published, lock_violations
...
def _wait_for_publish(published, timeout: float = _PUBLISH_TIMEOUT_S) -> None:
    ...
    Sleeping a fixed multiple of ``MIN_RENDER_INTERVAL`` and then
...
def test_a_fast_statement_publishes_no_progress_badge(progress_probe):
    badges, published, lock_violations = progress_probe
    badges.arm_progress([], display_id="d", step=1, total=2, code="x = 1")
    badges.cancel_progress()
...
def test_a_slow_statement_publishes_one_badge_naming_itself(progress_probe):
    badges, published, lock_violations = progress_probe
    badges.arm_progress([], display_id="d", step=1, total=2, code="slow = f()")
    ...
    # test if the assertion ran after `cancel_progress()`; asserting
