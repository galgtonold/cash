...
class TestTieredBackendMetadataOnly:
    ...
    def test_set_metadata_only_waits_for_inflight_full_write(self, tmp_path):
        ...
        try:
            backend.set("key1", {"variables": {"df": 42}}, {"execution_time": 10.0, "outputs": ["df"]})
            assert writing.wait(2), "full write should have started"

            done = threading.Event()

            def call_set_metadata_only():
                backend.set_metadata_only("key1", {"execution_time": 0.1, "metadata_only": True})
                done.set()
            ...
            assert not done.wait(0.5), "set_metadata_only must wait for the in-flight full write"
