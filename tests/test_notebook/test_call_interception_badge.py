...
def test_text_badge_marks_an_intercepted_group():
    ...
    assert "[intercepted]" in text, f"an intercepted group must be labelled as such:\n{text}"
...
def test_text_badge_leaves_a_decorated_group_unmarked():
    ...
    assert "compute()" in text, text
    assert "[intercepted]" not in text, f"a hand-decorated group was mislabelled as intercepted:\n{text}"
...
def test_html_badge_marks_an_intercepted_group():
    ...
    assert "@intercepted" in _html(intercepted=True), "the HTML badge does not distinguish an intercepted call"
