...
def test_a_read_over_a_list_of_paths_resolves_to_its_elements():
    ...
    assert statement_read_paths(
        "t = [pd.read_csv(f) for f in [DATA / 'a.csv', 'b.csv']]", namespace=NS) == {
        os.path.join("data", "a.csv"), "b.csv"}
...
def test_a_read_over_an_unknown_list_stays_unknown():
    ...
    assert statement_read_paths("t = [pd.read_csv(f.name) for f in TF]", namespace=dict(
        NS, TF=[Path("a.csv")])) is None
...
def test_a_loop_variable_also_bound_elsewhere_stays_unknown():
    ...
    assert statement_read_paths(
        "for f in TF:\n    f = f.with_suffix('.tsv')\n    x = pd.read_csv(f)", namespace=ns) is None
