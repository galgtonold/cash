...
class TestHandleLineageMismatch:
    ...
    def _call_mismatch(self, checker, var_name="x", actual="actual", virtual="virtual",
        ...
        if notebook_cells is None:
            notebook_cells = []
        checker._handle_lineage_mismatch(
            var_name=var_name,
...
class TestResolveInputLineage:
    ...
    def test_virtual_lineage_first(self):
        ...
        result = checker._resolve_input_lineage(
    ...
    def test_falls_back_to_variable_lineage(self):
        ...
        checker = _make_checker()
        checker.variable_lineage["x"] = "runtime_hash"
        result = checker._resolve_input_lineage("x", {}, set())
...
class TestResolveVirtualInputLineages:
    ...
    def test_basic_resolution(self):
        ...
        result = checker._resolve_virtual_input_lineages(
            "x = a + b", {"a", "b"}, {}, set()
        )
        assert len(result) == 2
    ...
    def test_empty_inputs(self):
        checker = _make_checker()
        result = checker._resolve_virtual_input_lineages("x = 1", set(), {}, set())
        assert result == []
...
class TestStatFileDeps:
    ...
    def test_existing_files(self, tmp_path):
        f = tmp_path / "data.csv"
        f.write_text("a,b\n1,2")
        result = UpstreamChecker._stat_file_deps({str(f): 0.0})
    ...
    def test_missing_files_excluded(self, tmp_path):
        ...
        result = UpstreamChecker._stat_file_deps({missing: 0.0})
        assert missing not in result

    def test_empty_input(self):
        result = UpstreamChecker._stat_file_deps({})
        assert result == {}
    ...
    def test_multiple_files(self, tmp_path):
        ...
        f1.write_text("data1")
        f2.write_text("data2")
        result = UpstreamChecker._stat_file_deps({str(f1): 0.0, str(f2): 0.0})
        assert len(result) == 2
...
class TestCheckFileDepsForRestore:
    ...
    def test_all_fresh(self, tmp_path):
        ...
        result = checker._check_file_deps_for_restore(
    ...
    def test_empty_deps(self):
        checker = _make_checker()
        result = checker._check_file_deps_for_restore({}, time.time())
        assert result is None
...
class TestCheckLineageConsistency:
    ...
    def test_consistent_lineage(self):
        ...
        result = checker._check_lineage_consistency(
...
class TestGetMetadataOnly:
    ...
    def test_no_cash_instance(self):
        checker = _make_checker()
        result = checker._get_metadata_only("some_key")
        assert result is None
    ...
    def test_with_get_metadata_method(self):
        ...
        result = checker._get_metadata_only("test_key")
...
class TestResetCaches:
    ...
    def test_clears_all_caches(self):
        checker = _make_checker()
        checker._ast_cache["code"] = MagicMock()
        checker._simulation_cache.append(MagicMock())
        checker._simulation_cell_hashes[0] = "hash"
        checker.reset_caches()
        assert len(checker._ast_cache) == 0
        assert len(checker._simulation_cache) == 0
        assert len(checker._simulation_cell_hashes) == 0
...
class TestResolveFallbackCacheIdx:
    ...
    def test_returns_none_for_cell_at_index_zero(self):
        ...
        checker = _make_checker()
        checker.last_cell_index = None
        checker._cell_id_to_last_index["cell_0"] = 0
    ...
    def test_returns_previous_cache_index(self):
        checker = _make_checker()
        checker.last_cell_index = None
        checker._cell_id_to_last_index["cell_2"] = 2
        # Need at least 2 simulation cache entries
        checker._simulation_cache = [MagicMock(), MagicMock(), MagicMock()]
    ...
    def test_uses_last_cell_index_without_cell_id(self):
        ...
        checker._simulation_cache = [MagicMock()] * 4
        result = checker._resolve_fallback_cache_idx(None)
        assert result == 2
    ...
    def test_cache_too_small(self):
        ...
        checker = _make_checker()
        checker.last_cell_index = None
        checker._cell_id_to_last_index["cell_5"] = 5
        checker._simulation_cache = [MagicMock()]  # Only 1 entry
...
class TestHandleDownstreamAdvancementFallback:
    ...
    def test_no_op_without_overlap(self):
        checker = _make_checker()
        checker._simulation_cache = [MagicMock()]
        checker._handle_downstream_advancement_fallback(
...
class TestComputeModuleSourceHash:
    ...
    def test_no_function_tracker(self):
        ...
        result = checker._compute_module_source_hash({"os"})
        assert result == ""
    ...
    def test_non_module_output(self):
        ...
        mock_tracker = MagicMock()
        checker.function_tracker = mock_tracker
        result = checker._compute_module_source_hash({"x"})
        assert result == ""
    ...
    def test_module_not_tracked(self):
        ...
        checker.function_tracker = mock_tracker
        result = checker._compute_module_source_hash({"fake_mod"})
        assert result == ""
    ...
    def test_module_with_source(self, tmp_path):
        ...
        checker.function_tracker = mock_tracker
        result = checker._compute_module_source_hash({"my_module"})
...
class TestHashModuleWithDeps:
    ...
    def test_basic_module_hash(self, tmp_path):
        ...
        mock_tracker = MagicMock()
        mock_tracker._dep_file_to_parents = {}
        result = checker._hash_module_with_deps("mod", str(mod_file), mock_tracker)
        assert result.startswith(":mod_src:")
    ...
    def test_hash_includes_dep_files(self, tmp_path):
        ...
        h1 = checker._hash_module_with_deps("mod", str(mod_file), mock_tracker)
        # Change dep file
        dep_file.write_text("def util(): return 42")
        h2 = checker._hash_module_with_deps("mod", str(mod_file), mock_tracker)
        assert h1 != h2
    ...
    def test_missing_module_file(self, tmp_path):
        ...
        mock_tracker = MagicMock()
        mock_tracker._dep_file_to_parents = {}
        result = checker._hash_module_with_deps("mod", str(tmp_path / "nofile.py"), mock_tracker)
        assert result == ""
