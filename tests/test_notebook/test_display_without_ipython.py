"""Rich output without IPython must fail LOUDLY, never silently.
...
2. **A genuine display attempt must RAISE** — ``processor.py`` used to satisfy
   ...
   has. ``restore.py`` already did the opposite (a function-local import), and
   ``processor.py`` now matches.
...
_SCRIPT = textwrap.dedent(
        ...
        def find_spec(self, fullname, path=None, target=None):
            if fullname.split(".")[0] == "IPython":
                raise ModuleNotFoundError(
                    "No module named 'IPython' (blocked by the no-IPython guard)",
                    name=fullname,
                )
            return None
    ...
    # --- Phase 3: restore.py's replay must RAISE too (parity) ------------
    try:
...
def test_restore_rich_output_raises_without_ipython(no_ipython_run):
    """The sibling module behaves the same way — the two modules agree."""
