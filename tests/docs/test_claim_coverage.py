...
#   calls-inside-body    every call is inside a function or class body, which
#                        inference skips because such code usually does not run.
#                        For the async pages it *does* run (`asyncio.run(main())`),
#                        so teaching inference to follow a top-level-invoked
#                        function would fix those legitimately -- see CAS-239.
#
