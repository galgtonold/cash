...
def test_a_groupby_aggregation_is_a_new_frame(cash_instance):
    @cash_instance.cache
    def totals(df):
...
def test_a_transform_and_an_apply_are_new(cash_instance):
    @cash_instance.cache
    def shaped(df):
...
def test_numpy_builders_are_new(cash_instance):
    @cash_instance.cache
    def joined(a):
...
def test_sorted_returns_a_new_list(cash_instance):
    @cash_instance.cache
    def ordered(df):
...
def test_a_real_argument_mutation_still_warns(cash_instance):
    @cash_instance.cache
    def stamp(df):
        df["seen"] = 1
        return len(df)
...
def test_a_real_global_mutation_still_warns(cash_instance):
    counter: dict = {}
...
def test_logging_through_get_logger_is_a_diagnostic_line(cash_instance):
...
def test_a_print_to_stdout_still_says_so(cash_instance):
