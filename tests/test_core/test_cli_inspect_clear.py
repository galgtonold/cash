...
def _write_entry(cache_dir, key, payload=b"x" * 100, **meta):
    """An entry as the file backend writes it, named by its key's SHA-256; its id."""
...
def test_an_entry_is_sized_by_meta_plus_data(tmp_path):
    cache = tmp_path / ".cash"
    _write_entry(cache, "mod.f:1:2:3", payload=b"y" * 500)
...
def test_unreadable_metadata_is_skipped_not_fatal(tmp_path):
    ...
    cache = tmp_path / ".cash"
    _write_entry(cache, "mod.f:1:2:3")
...
def test_inspect_groups_by_function(tmp_path):
    cache = tmp_path / ".cash"
    _write_entry(cache, "mod.big:1:2:3", payload=b"x" * 4000)
    _write_entry(cache, "mod.big:9:2:3", payload=b"x" * 4000)
    _write_entry(cache, "mod.small:1:2:3", payload=b"x" * 10)
...
def test_clear_function_removes_only_that_function(tmp_path):
    cache = tmp_path / ".cash"
    doomed = _write_entry(cache, "mod.doomed:1:2:3")
    _write_entry(cache, "mod.kept:1:2:3")
    ...
    assert not (cache / f"{doomed}{ENTRY_SUFFIX}").exists(), "the entry outlived the clear"
...
def test_the_entry_view_shows_what_an_entry_is_worth(tmp_path):
    ...
    cache = tmp_path / ".cash"
    _write_entry(cache, "mod.f:1:2:3", payload=b"x" * 5000, execution_time=0.2, access_count=1)
    _write_entry(cache, "mod.f:9:2:3", payload=b"x" * 100, execution_time=41.2, access_count=7)
...
def test_notebook_entries_name_the_variables_they_produced(tmp_path):
    ...
    cache = tmp_path / ".cash"
    _write_entry(cache, "stmt:9f8e", outputs=["df", "model"], execution_time=12.5)
...
def test_one_entry_can_be_dropped_by_id_prefix(tmp_path):
    ...
    cache = tmp_path / ".cash"
    dropped = _write_entry(cache, "mod.f:1:2:3")
    kept = _write_entry(cache, "mod.f:9:2:3")
    assert dropped[:6] != kept[:6]
    result = _cli("clear", str(cache), "--entry", dropped[:6], cwd=tmp_path)
    assert result.returncode == 0, result.stdout
    assert [e.stem for e in _scan_entries(cache)] == [kept]
    assert not (cache / f"{dropped}{ENTRY_SUFFIX}").exists(), "the entry outlived the clear"


def test_an_ambiguous_entry_prefix_refuses_and_lists(tmp_path):
    """Deleting the wrong entry is not recoverable, so never guess."""
    cache = tmp_path / ".cash"
    # Two keys whose ids start alike.
    keys = (f"mod.f:{i}:2:3" for i in itertools.count())
    first_chars: dict[str, str] = {}
    for key in keys:
        stem = hashlib.sha256(key.encode("utf-8")).hexdigest()
        if stem[0] in first_chars:
            break
        first_chars[stem[0]] = key
    _write_entry(cache, first_chars[stem[0]])
    _write_entry(cache, key)
    result = _cli("clear", str(cache), "--entry", stem[0], cwd=tmp_path)
