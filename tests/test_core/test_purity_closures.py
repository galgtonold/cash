...
def test_impure_closure_helper_flagged(analyzer):
    ...
    assert any(i.kind == ISSUE_IMPURE_CALL and "os.system" in i.description for i in r.issues), r.format()
...
def test_closure_helper_source_hash_captured(analyzer):
    ...
    for q in closure_quals:
        assert q not in r.helper_resolution_paths, f"closure helper {q!r} should not have a resolution path"
...
def test_nested_closure_recursion(analyzer):
    ...
    main = make()
    r = analyzer.analyze(main)
    assert any(i.kind == ISSUE_IMPURE_CALL and "requests.post" in i.description for i in r.issues), r.format()
