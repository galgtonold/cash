...
Every cached call appends an event to ``CallLog.entries`` for the
...
import cash.decorator.reporting as reporting
from cash import Cash
from cash.backends.memory_backend import InMemoryBackend


def test_the_log_keeps_only_the_most_recent_calls(monkeypatch):
    monkeypatch.setattr(reporting, "CALL_LOG_MAX", 50)
    ...
    assert len(c._calls.entries) <= 50
