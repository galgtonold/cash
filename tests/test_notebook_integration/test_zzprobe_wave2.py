...
def test_env_var_edit_invalidates_reader(nb_runner):
    ...
    assert "factor= 99" in out, f"env-var edit not propagated: consumer served stale factor. Got {out!r}"
...
def test_decimal_context_edit_invalidates(nb_runner):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(2)
    assert "r= 0.14285714" in out, f"decimal-context edit not propagated: consumer served stale precision. Got {out!r}"
...
def test_monkeypatch_class_method_edit(nb_runner):
    ...
    assert "scaled= 15" in out, f"edited monkeypatch not picked up by isolated call-site re-run. Got {out!r}"
...
def test_conditional_def_flag_edit(nb_runner):
    ...
    nb_runner.run_cell(3)
    out = nb_runner.get_output(3)
    assert "f()= 2" in out, f"flag edit did not swap the conditionally-defined function. Got {out!r}"
...
def test_class_method_edit_invalidates_instance_call(nb_runner):
    ...
    nb_runner.set_cell_source(1, "class Model:\n    def predict(self, x):\n        return x + 100")
    nb_runner.run_cell(3)
    out = nb_runner.get_output(3)
    assert "pred= 101" in out, f"edited method body not seen by isolated re-run of the call site. Got {out!r}"
...
def test_starred_unpack_alias_rerun_idempotent(nb_runner):
    ...
    out = nb_runner.get_output(2)
    assert "rest= [2, 3, 4, 9]" in out, f"starred-unpack re-run not idempotent: {out!r}"
...
def test_exception_object_crossing_cells(nb_runner):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(2)
    assert "etype= ZeroDivisionError" in out, f"exception-holding var broke after restart+persist: {out!r}"
...
def test_run_magic_script_edit_invalidates(nb_runner, tmp_path):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(2)
    assert "K2= 100" in out, f"%run script edited on disk but consumer served stale K. Got {out!r}"
...
def test_time_cell_edit_gives_fresh_stamp(nb_runner):
    ...
    assert s1 != s2, f"edited time-stamp cell served a STALE stamp: {s1!r} == {s2!r}"
...
def test_user_class_instances_restart_persist(nb_runner):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(3)
    assert "sx= 4" in out, f"__main__-class instances broke after restart+persist: {out!r}"
...
def test_lambda_crossing_cells_restart_persist(nb_runner):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(2)
    assert "sq4= 16" in out, f"lambda-valued var broke after restart+persist: {out!r}"
