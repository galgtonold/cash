...
def test_skipped_statement_no_spurious_output(nb_runner):
    ...
    assert "x" not in output1 or output1.strip() == "", f"SKIPPED should not output variable names, got: {output1!r}"

    nb_runner.run_cell(2)
    output2 = nb_runner.get_output(2)
    assert "y" not in output2 or output2.strip() == "", f"SKIPPED should not output variable names, got: {output2!r}"
...
try:
    ...
    assert "result" in html and "status" in html, f"Badge should show body statements for try/except, got: {html[:500]}"
