...
def arm(label: str, body: str, effect: str, *, global_probe: bool) -> None:
    ...
    runner.create_notebook(
        [
            ...
            "import os\nimport numpy as np\n"
            f"LOG = r'{log}'\nSEEN = []\n"
            "def price(vol):\n" + body.format(n=N_PATHS) + effect + "    return float(est)",
            "vols = [0.10, 0.15, 0.20]",
            "out = []\nfor v in vols:\n    out.append(price(v))",
        ]
    )
    ...
    print(f"  {label}")
    print(
        f"     cold {unit}={before}   after reorder: +{got}   "
        f"{'*** SIDE EFFECT DROPPED ***' if got == 0 else 'side effect fired'}"
    )


print(
    "Reordering [0.10,0.15,0.20] -> [0.15,0.10,0.20]. The three values are all\n"
...
arm("A  file write (builtins.open), 2D + axis reduction  [P3's shape]", BODY_2D, WRITE_OPEN, global_probe=False)
arm("B  global-list mutation,      2D + axis reduction  [P3's control]", BODY_2D, MUTATE_GLOBAL, global_probe=True)
arm("C  file write (builtins.open), 1D array", BODY_1D, WRITE_OPEN, global_probe=False)
arm("D  file write (os.write),      2D + axis reduction", BODY_2D, WRITE_OS, global_probe=False)
