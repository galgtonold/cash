...
SORT_CELL = "df = df.sort_values('val', ascending=False)\nprint('sorted:', df['val'].tolist())"


SUM_CELL = "total = df['val'].sum()\nprint('total =', total)"
...
class TestFileDepSimulationCache:
    ...
    def test_file_change_without_self_assignment(self, nb_runner, tmp_path):
        ...
        assert "total = 600" in out2, (
            f"Without self-assignment, file change should still be detected. Expected total=600, got: {out2}"
        )
    ...
    def test_file_change_detected_on_second_downstream_run(self, nb_runner, tmp_path):
        ...
        assert "total = 6000" in out2, (
            f"Second downstream run after file change should detect staleness. Expected total=6000, got: {out2}"
        )
