...
def test_runall_does_not_double_a_loop_fed_audit_append(nb_runner, tmp_path):
    ...
    assert _rows(audit) == 1, (
        f"run-all re-fired the loop-fed audit append: audit.log has {_rows(audit)} lines (expected 1)"
    )
...
def test_restart_reader_does_not_refire_co_produced_writer(nb_runner, tmp_path):
    ...
    assert _rows(audit) == 1, f"post-restart reader re-fired the audit append: {_rows(audit)} lines (expected 1)"
