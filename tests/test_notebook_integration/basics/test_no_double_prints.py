...
def _count(output: str, marker: str) -> int:
    ...
    lines = [line for line in output.splitlines() if not line.startswith("[") and "Cash:" not in line]
    clean = "\n".join(lines)
...
def _assert_single(output: str, *markers: str):
    ...
    for m in markers:
        ...
        assert count == 1, f"'{m}' appeared {count} times (expected 1). Output:\n{output}"
...
class TestNoDoublePrintsFinancialPattern:
    ...
    def test_financial_demo_multi_computation(self, nb_runner, tmp_path):
        ...
        _assert_single(out, "CALC_VOL_START", "CALC_VOL_END", "CALC_SMA_START", "CALC_SMA_END")
