"""Adversarial probes: randomness, annotations, cell
magics, stdlib accumulators, cycles, decorator persistence across restart.

 1. test_unseeded_random_not_served_stale    — ADJUDICATED: freezing
...
def test_unseeded_random_not_served_stale(nb_runner):
    """ADJUDICATED: freezing is intended; the opt-out is the contract.
    ...
    assert v1 != v2, (
        f"# @cash:no-cache must redraw each run: {v1} == {v2} (it has to switch off the RNG rewind, not just caching)"
    )
...
def test_allow_random_annotation_caches(nb_runner):
    ...
    assert v1 == v2, (
        f"# @cash:allow-random ignored: annotated unseeded cell re-drew on an unchanged rerun ({v1} vs {v2})"
    )
...
def test_ttl_annotation_within_ttl_cached(nb_runner):
    ...
    assert MISS not in raw, (
        f"# @cash:ttl=60 statement recomputed within ttl: {[l for l in raw.splitlines() if MISS in l]}"
    )
...
def test_ttl_annotation_expiry(nb_runner):
    ...
    raw = nb_runner.get_raw_output(1)
    assert MISS in raw, (
        "# @cash:ttl=1 did not expire: no recompute 1.5s after the entry was written (annotation ignored?)"
    )
...
def test_capture_cellmagic_graceful(nb_runner):
    ...
    assert "y5= 14" in out, f"edit did not propagate through a %%capture cell: {out!r}"
...
def test_writefile_cellmagic_chain(nb_runner, tmp_path):
    ...
    assert "b= hello v2" in out, (
        f"reader served stale content after the %%writefile cell was edited (run_all!): {out!r}"
    )
...
def test_counter_defaultdict_rerun_idempotent(nb_runner):
    ...
    out = nb_runner.get_output(2)
    assert expected in out, f"Counter/defaultdict accumulators not idempotent on isolated re-run: {out!r}"
...
def test_cyclic_structure_cells_restart(nb_runner):
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(2)
    assert "n= 3 self= True" in out, f"cyclic structure broke after restart+persist: {out!r}"
