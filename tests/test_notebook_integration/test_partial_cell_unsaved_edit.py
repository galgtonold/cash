...
def test_partial_cache_after_unsaved_edit(nb_runner):
    ...
    nb_runner.nb.cells[
        1
    ].source = "df['VolAdj'] = df['Close'] * df['Volume']\ndf['SMA_61'] = df['Close'].rolling(61).mean()"
...
def test_partial_cache_debug_output_shows_fallback(nb_runner):
    ...
    assert "Current cell not found in notebook" in raw_output or "Downstream advancement fallback" in raw_output, (
