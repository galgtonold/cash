...
@LOAD_SENSITIVE
def test_a_comment_added_to_a_module_does_not_re_run_its_callers(nb_runner, tmp_path):
    raw = _run(nb_runner, tmp_path, "fmtlib", "import fmtlib", "fmtlib", _module("# a note to myself\n"))
    assert "CACHED: DATA" in raw, (
        "a comment was added to the module and the call re-ran; nothing it does changed:\n" + raw
    )


@LOAD_SENSITIVE
def test_the_same_through_an_alias(nb_runner, tmp_path):
    raw = _run(nb_runner, tmp_path, "fmtalias", "import fmtalias as fa", "fa", _module("# a note to myself\n"))
    assert "CACHED: DATA" in raw, "a comment was added and the call re-ran, reached through an alias:\n" + raw


@LOAD_SENSITIVE
def test_a_docstring_reworded_in_a_module_does_not_re_run_its_callers(nb_runner, tmp_path):
    documented = _module().replace("def load(n):\n", 'def load(n):\n    """Load n rows."""\n')
    ...
    mod.write_text(documented.replace("Load n rows.", "Return n rows, as strings."), encoding="utf-8")
    nb_runner.run_cell(3)
    raw = nb_runner.get_raw_output(3)
    assert "CACHED: DATA" in raw, "only a docstring in the module changed and the call re-ran:\n" + raw
...
def test_a_docstring_reworded_in_a_cell_does_not_re_run_its_callers(nb_runner):
    ...
    raw = nb_runner.get_raw_output(3)
    assert "CACHED: DATA" in raw, "only the docstring of the function changed and its caller re-ran:\n" + raw
...
def test_an_edit_to_the_function_it_calls_still_re_runs_it(nb_runner, tmp_path):
    ...
    raw = _run(nb_runner, tmp_path, "fmtreal", "import fmtreal", "fmtreal", changed)
    assert "CACHED: DATA" not in raw, (
        "the function the statement calls was edited and it was served from cache anyway:\n" + raw
    )
...
def test_a_cash_directive_added_to_a_module_still_re_runs_it(nb_runner, tmp_path):
    ...
    raw = _run(nb_runner, tmp_path, "fmtdirective", "import fmtdirective", "fmtdirective", annotated)
    assert "CACHED: DATA" not in raw, (
