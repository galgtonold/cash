...
def test_pandas_pipeline_edit_midstream(nb_runner, tmp_path):
    ...
    nb_runner.set_cell_source(4, "feat = clean.assign(revenue=clean.units * clean.price * 0.9)")
    ...
    nb_runner.run_all()
    out = nb_runner.get_output(6)
    assert "summary= {'north': 72.0, 'south': 99.0}" in out, f"run_all after feature edit wrong: {out!r}"
...
def test_pandas_pipeline_downstream_edit_no_upstream_recompute(nb_runner, tmp_path):
    ...
    nb_runner.set_cell_source(6, "print('SUMMARY:', {k: round(v, 1) for k, v in sorted(summary.items())})")
    nb_runner.run_cell(6)
    out = nb_runner.get_output(6)
    ...
    assert not heavy_reexec, (
        f"EFFECTIVENESS: editing only the report cell re-executed upstream pipeline statements: {heavy_reexec}"
    )
...
def test_finance_backtest_param_edit(nb_runner):
    ...
    assert "vols= {2:" in out and "4:" in out, f"backtest param edit not propagated through the loop: {out!r}"
...
def test_numpy_training_loop_lr_edit(nb_runner):
    ...
    assert _res(out2) != _res(out1), f"lr edit produced identical training result: {out2!r}"

    # Revert: must reproduce the original numbers exactly.
    nb_runner.set_cell_source(2, "lr = 0.01")
    nb_runner.run_all()
    out3 = nb_runner.get_output(4)
    assert _res(out3) == _res(out1), f"lr revert did not reproduce original training result: {out3!r} vs {out1!r}"
...
def test_etl_write_read_chain_param_edit(nb_runner, tmp_path):
    ...
    assert "total= 600" in out, (
        f"ETL param edit did not propagate through the to_csv/read_csv file boundary on run_all: {out!r}"
    )
...
def test_matplotlib_figure_cell_rerun(nb_runner, tmp_path):
    ...
    assert Path(png).exists(), "figure cell served from cache without re-creating the savefig output"
...
def test_seeded_montecarlo_seed_edit(nb_runner):
    ...
    assert est3 == est1, f"seed revert did not reproduce the original estimate: {est3!r} vs {est1!r}"
