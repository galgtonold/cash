...
class TestFastModeCacheIntegration:
    ...
    def test_loop_second_run_no_fast_mode(self, nb_runner):
        ...
        assert "FAST MODE" not in output2, f"FAST MODE should not trigger on cached iterations. Output: {output2}"
    ...
    def test_financial_pattern_loop_cached(self, nb_runner):
        ...
        assert "FAST MODE" not in output2, f"Financial pattern loop should be cached, not fast-moded. Output: {output2}"
    ...
    def test_extending_loop_iterable_uses_per_iteration(self, nb_runner):
        ...
        assert "FAST MODE" not in output2, (
            f"Extended loop should use per-iteration caching, not fast mode. Output: {output2}"
        )
