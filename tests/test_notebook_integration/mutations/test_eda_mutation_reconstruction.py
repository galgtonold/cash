...
def _assert_matches_oracle(scenario, tmp_path):
    ...
    assert not diffs, f"cash-ON diverged from top-to-bottom oracle in {scenario.__name__}:\n" + "\n".join(
        f"  [{k}] oracle={o!r}  cash={c!r}" for k, (o, c) in diffs.items()
    )
...
@pytest.mark.timeout(300)
@pytest.mark.xfail(
    reason="documented limitation (docs/known-limitations.md: "
    "'Mutation that cash cannot see'); isolated re-run only. "
    "Serves as the harness's non-vacuity proof.",
    strict=True,
)
@pytest.mark.parametrize("scenario", CANARY_SCENARIOS, ids=[s.__name__ for s in CANARY_SCENARIOS])
