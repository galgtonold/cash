...
class TestLoopProducedVariableUpstream:
    ...
    def test_loop_variable_restored_after_overwrite(self, nb_runner):
        ...
        assert "item_data=30" in output, f"Loop-produced variable should be restored by upstream. Got: {output}"
    ...
    def test_loop_variable_not_stale_after_intervening_cell(self, nb_runner):
        ...
        nb_runner.run_cell(2)
        output = nb_runner.get_output(2)
        assert "result=110" in output, f"Expected result=110 from loop-produced val. Got: {output}"
    ...
    def test_loop_variable_updated_when_loop_changes(self, nb_runner):
        ...
        nb_runner.run_cell(2)
        output = nb_runner.get_output(2)
        assert "last_item=300" in output, f"Loop-produced variable should reflect updated loop code. Got: {output}"
    ...
    def test_upstream_loop_reexecution_uses_per_iteration_cache(self, nb_runner):
        ...
        assert "item_data=30" in output, f"Loop-produced variable should be restored. Got: {output}"
        # If per-iteration cache is used, this should take < 1s total
        # (vs 1.5s+ if all 3 iterations recompute with 0.5s sleep each)
        assert elapsed < 3.0, (
            f"Upstream loop re-execution should use per-iteration cache. Took {elapsed:.1f}s (expected < 3s)"
        )
