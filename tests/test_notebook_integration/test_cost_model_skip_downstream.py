...
try:
    ...
    if _cash_magics:
        ...
        if hasattr(_cash_magics, '_statement_processor'):
            _cash_magics._statement_processor.tracking_state.lineage.clear()
...
def test_floor_skipped_upstream_downstream_hits_after_reset(nb_runner):
    ...
    assert statuses & {"restored", "skipped"}, (
        f"Cell 3's badge has no restored/skipped indicator — could not verify cache reuse. Statuses found: {statuses}"
    )
...
def test_two_floor_skipped_layers_downstream_still_hits(nb_runner):
    ...
    assert out_warm == out_cold, f"Downstream value diverged across reset.\n  cold: {out_cold!r}\n  warm: {out_warm!r}"
    ...
    assert statuses & {"restored", "skipped"}, f"Cell 4's badge has no restored/skipped indicator. Statuses: {statuses}"
