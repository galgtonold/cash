...
def _arm(nb, ms, floor0, cash_on=True):
    cells = [ON, FLOOR0, _defs(ms), LOOP, SINK] if floor0 else [(ON if cash_on else OFF), _defs(ms), LOOP, SINK]
...
def test_body_0_1ms(nb_runner):
    r = {"oracle": _arm(nb_runner, 0.1, False, cash_on=False), "floor0": _arm(nb_runner, 0.1, True)}
    print("F_0_1MS " + json.dumps(r))


def test_body_1ms(nb_runner):
    r = {"oracle": _arm(nb_runner, 1, False, cash_on=False), "floor0": _arm(nb_runner, 1, True)}
    print("F_1MS " + json.dumps(r))


def test_body_2ms(nb_runner):
    r = {"oracle": _arm(nb_runner, 2, False, cash_on=False), "floor0": _arm(nb_runner, 2, True)}
