...
pytestmark = [pytest.mark.timeout(120), pytest.mark.restore, pytest.mark.fresh_kernel]
...
def test_class_cell_survives_restart_with_persist(nb_runner):
    ...
    nb_runner.enable_persist()
    nb_runner.run_all()
    assert "sx= 4" in nb_runner.get_output(3), "class-defining notebook broke after restart+persist"
...
def test_def_plus_unpicklable_user_survives_restart(nb_runner):
    ...
    nb_runner.enable_persist()
    nb_runner.run_all()
    assert "res=42" in nb_runner.get_output(2), "def+executor cell broke after restart+persist"
...
def test_lambda_assign_survives_restart(nb_runner):
    ...
    nb_runner.enable_persist()
    nb_runner.run_all()
    assert "sq4= 16" in nb_runner.get_output(2), "lambda-valued var broke after restart+persist"
...
def test_function_alias_survives_restart(nb_runner):
    ...
    nb_runner.enable_persist()
    nb_runner.run_all()
    assert "a= 2" in nb_runner.get_output(3), "function-alias chain broke after restart+persist"
