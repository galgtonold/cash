"""Finding unseeded randomness in a statement's source, and saying so.

The AST visitor, the session-scoped :class:`RandomnessDetector` (module seeds
and RNG carriers), the ``RANDOM-*`` warning text and emitters, and the
per-statement answers "which RNG modules does this code draw from / seed /
re-seed from entropy" that the cache key and the lineage are built on.
"""
...
# -----------------------------------------------------------------------------
# Which modules a statement draws from, seeds, or re-seeds from entropy
# -----------------------------------------------------------------------------
