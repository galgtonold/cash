"""Per-variable computation history behind ``%cash_provenance``."""
...
class ProvenanceTracker:
    """The session's per-variable computation history, as ``%cash_provenance``
    shows it.

    Each variable keeps its last ``max_history_per_var`` records: the code that
    produced it, its inputs, status and duration. Values are not stored, so
    this explains how a value was made; it cannot bring an old one back.
    """
    ...
    def _all_recorded_inputs(self, variable: str) -> list[str]:
        ...
        graph needs the union to surface a meaningful chain.
        """
        seen: set[str] = set()
        out: list[str] = []
    ...
    def _dependents(self, variable: str) -> set[str]:
        """Variables whose latest record reads *variable*."""
        dependents = set()
    ...
    def _format_graph_section(self, variable: str, max_depth: int = 5) -> list:
        ...
        dependents = self._dependents(variable) - {variable}
        if dependents:
    ...
    def to_json(self, variable: str) -> str:
        """Export one variable's history as JSON."""
        return json.dumps([r.to_dict() for r in self.get_history(variable)], indent=2, default=str)
