...
class S3Backend(CacheBackend):
    ...
    def _ranged_get(self, obj_key: str, nbytes: int) -> bytes:
        ...
        obj = self.s3.get_object(Bucket=self.bucket, Key=obj_key, Range=f"bytes=0-{nbytes - 1}")
    ...
    def set_metadata_only(self, key: str, metadata: dict) -> None:
        ...
        try:
            self.s3.put_object(Bucket=self.bucket, Key=obj_key, Body=pack_entry(metadata, b""))
        except self.botocore_exceptions.ClientError as e:
    ...
    def delete(self, key: str) -> None:
        ...
        try:
            ...
            raise CacheBackendError(f"S3 delete failed for key '{key}': {e}") from e
    ...
    def clear(self) -> None:
        ...
        try:
            ...
            raise CacheBackendError(f"S3 clear failed for prefix '{self.prefix}': {e}") from e
